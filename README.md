...
├── operations.json           # Lightweight operation summary (for tooling/AI)
└── OPERATIONS.md             # Human/AI readable API reference
...
- `output/operations.json` - Lightweight operation summary for tooling
- `output/OPERATIONS.md` - Human/AI readable API reference
- `fixtures/**/*.json` - Test fixtures for mocking API responses

## AI/Tooling Summaries

The spec includes lightweight summaries designed for AI assistants and tooling:

### operations.json

A compact JSON file containing all 59 operations with their metadata:
  ...
  "operations": [
    {
      ...
      "requiredFields": ["name"],
      "optionalFields": ["assignToken", "variables", "securityProperties", "description"],
      "responseType": "object"
    }
  ],
  "byTag": {
...
This is much smaller than the full OpenAPI spec (~60KB vs ~500KB) and contains:
- All operation metadata (method, path, parameters)
- Request body field names and types
- Response types
- Organized both as a flat list and grouped by tag

### OPERATIONS.md

A markdown file with the same information in human-readable format:
- Table of contents by category
- Detailed reference for each operation
- Required vs optional request body fields
- Parameter documentation

These summaries are auto-generated by `npm run summarize` (included in `npm run build`).
